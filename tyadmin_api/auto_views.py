...
class CouponViewSet(XadminViewSet):
    ...
    def get_serializer_class(self):
        ...
        if self.action == "list":
            print("self.request::", self.request.user.has_perm('app_api.change_coupon'))
            print("self.request::", self.request.user.has_perm('app_api.add_coupon'))
            # add_footer
            print("self.request::", self.request.user.has_perm('app_api.add_footer'))
            return CouponListSerializer
        else:
            # if self.request.user.has_perm('app_api.add_coupon'):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            # del_dict = {}
            print("has_perm, add_coupon::", request.user.has_perm('app_api.add_coupon'))
            if not request.user.has_perm('app_api.add_coupon'):
                print("has_perm, add_coupon::", request.user.has_perm('app_api.add_coupon'))
                # return HttpResponse("{'status':405, 'msg':'添加出错'}", content_type='application/json')
                return Response({'msg': '添加出错'}, status=status.HTTP_403_FORBIDDEN)
...
class UserViewSet(XadminViewSet):
    ...
    def get_serializer_class(self):
        if self.action == "list":
            ...
            print("UserViewSet.....:", self.request.data)
            # 'user_permissions': ['469', '470']} >
